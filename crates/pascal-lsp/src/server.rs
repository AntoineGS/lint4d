...
use crate::workspace::rename::{self, SourceRecord, WorkspaceInput};
use crate::workspace::{
...
impl AnalysisJobs {
    ...
    fn start(
        ...
        self.start_immediately(id, request, input, features, cancellation)
    }
...
mod tests {
    ...
    #[test]
    fn analysis_workers_reject_requests_beyond_the_worker_bound() {
        let temp = tempfile::tempdir().expect("temporary workspace");
        let root = temp.path().join("fixture");
        ...
        let error = jobs
            .start(
                RequestId::from("busy-rejected".to_string()),
                AnalysisRequest::SemanticTokens {
            ...
            .expect_err("requests beyond the active worker bound must be rejected");
        assert_eq!(error, "analysis server is busy; retry the request");
        ...
        assert!(jobs.is_empty(), "both admitted requests must be drained");
    }
    ...
    fn delivery_rejects_computed_reference_and_highlight_results_after_a_project_switch() {
        ...
        deliver_successfully(&workspace, semantic_control);
        ...
        for result in [reference, highlight, semantic] {
            let (server, client) = Connection::memory();
            deliver_analysis_result(&server, &workspace, result).expect("deliver stale result");
