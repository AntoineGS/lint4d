...
use crate::rules::scope::{
    ...
    is_dot_rhs, is_inside_inherited, is_inside_module_name, is_inside_proc, is_inside_typeref,
    Scopes,
};
...
// ---------------------------------------------------------------------------
// Pass 2: collect edits (placeholder — implemented in Task 4)
// ---------------------------------------------------------------------------

fn collect_edits(
    _root: Node,
    _source: &[u8],
    _rename_map: &RenameMap,
    _scopes: &Scopes,
    _config: &Config,
) -> Vec<TextEdit> {
    Vec::new() // placeholder
...
// ---------------------------------------------------------------------------
// Edit application (placeholder — implemented in Task 5)
// ---------------------------------------------------------------------------

fn apply_edits(source: &[u8], edits: Vec<TextEdit>) -> Result<(Vec<u8>, usize), String> {
    if edits.is_empty() {
        return Ok((source.to_vec(), 0));
    }
    Ok((source.to_vec(), 0)) // placeholder
}
