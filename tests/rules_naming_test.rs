...
fn type_prefix_flags_missing_t() {
    ...
    let matches: Vec<_> = diagnostics.iter().filter(|d| d.rule_id == "type-prefix").collect();
    assert_eq!(matches.len(), 2, "Expected 2 type-prefix diagnostics for MyClass and BadRecord, got: {:?}", matches);
}

#[test]
fn interface_prefix_flags_missing_i() {
    let diagnostics = lint_fixture("tests/fixtures/naming/bad_interface_prefix.pas");
    let matches: Vec<_> = diagnostics.iter().filter(|d| d.rule_id == "interface-prefix").collect();
...
fn naming_passes_with_correct_prefixes() {
    ...
    let naming: Vec<_> = diagnostics.iter()
        .filter(|d| d.rule_id == "type-prefix" || d.rule_id == "interface-prefix")
        .collect();
    assert!(naming.is_empty(), "Expected no naming diagnostics, got: {:?}", naming);
}
...
fn naming_rules_skipped_for_dpr_files() {
    ...
    let naming: Vec<_> = diagnostics.iter()
        .filter(|d| d.rule_id == "type-prefix" || d.rule_id == "interface-prefix")
        .collect();
    assert!(naming.is_empty(), "Naming rules should be skipped for .dpr files");
}
